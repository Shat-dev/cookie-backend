...
async function runTests() {
    ...
    try {
        ...
        console.log(`  Initial state: ${isFirstRound ? "No rounds created yet" : "At least one round exists"}`);
        if (isFirstRound) {
        ...
        for (const test of testCases) {
            ...
            console.log(`  ${test.isFirst ? "First" : "Subsequent"} round: ${durationHours}h (${durationSeconds}s)`);
            if (durationHours === test.expectedHours) {
                console.log(`  ✅ Correct duration`);
            }
            else {
                console.log(`  ❌ Incorrect duration (expected ${test.expectedHours}h)`);
            }
        }
        console.log();
        ...
        console.log(`  Freeze starts at: ${new Date(freezeTime * 1000).toLocaleTimeString()}`);
        ...
        const testScenarios = [
            {
                name: "Before freeze",
                now: mockRound.end - FREEZE_SEC - 60,
                shouldFreeze: false,
                reason: "before-freeze",
            },
            {
                name: "In freeze window",
                now: mockRound.end - FREEZE_SEC + 30,
                shouldFreeze: true,
                reason: null,
            },
            {
                name: "After round end",
                now: mockRound.end + 60,
                shouldFreeze: false,
                reason: "after-end",
            },
        ];
        for (const scenario of testScenarios) {
            const inFreezeWindow = scenario.now >= mockRound.end - FREEZE_SEC &&
                scenario.now < mockRound.end;
            const shouldFreeze = mockRound.isActive && !mockRound.isCompleted && inFreezeWindow;
            console.log(`  ${scenario.name}: ${shouldFreeze
                ? "Should freeze"
...
//# sourceMappingURL=test-round-coordinator.js.map