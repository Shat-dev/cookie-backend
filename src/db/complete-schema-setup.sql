...
-- Add new columns to winners table if they don't exist
DO $$
BEGIN
  -- Add payout_amount column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'winners' AND column_name = 'payout_amount'
  ) THEN
    ALTER TABLE winners ADD COLUMN payout_amount VARCHAR(255);
  END IF;

  -- Add payout_status column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'winners' AND column_name = 'payout_status'
  ) THEN
    ALTER TABLE winners ADD COLUMN payout_status VARCHAR(20) DEFAULT 'pending' 
    CHECK (payout_status IN ('pending', 'success', 'failed'));
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'winners' AND column_name = 'payout_failure_reason'
  ) THEN
    ALTER TABLE winners ADD COLUMN payout_failure_reason TEXT;
  END IF;
END $$;
...
-- Add indexes for winners table after columns are added
CREATE INDEX IF NOT EXISTS idx_winners_payout_status ON winners(payout_status);
CREATE INDEX IF NOT EXISTS idx_winners_wallet_payout ON winners(winner_address, payout_status);
...
WHERE table_name IN ('lottery_rounds', 'lottery_winners') 
AND column_name IN ('funds_admin_address', 'draw_interval_hours', 'payout_amount', 'payout_status', 'payout_failure_reason')
