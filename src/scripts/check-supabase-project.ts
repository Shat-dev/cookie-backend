...
async function checkSupabaseProject() {
  ...
  // Extract project reference assuming format: db.{project-ref}.supabase.co
  let projectRef = "";
  ...
  const endpoints = [
    {
      ...
      expectedCodes: [200, 401, 403], // These indicate the service exists
    },
    {
      name: "Auth API",
      url: `https://${projectRef}.supabase.co/auth/v1/`,
      expectedCodes: [200, 404, 405], // 404/405 still means service exists
    },
    {
      name: "Storage API",
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (endpoint.expectedCodes.includes(response.status)) {
        ...
        console.log(
          `   ⚠️  ${endpoint.name} server error - project might exist but having issues`
        );
      } else {
        console.log(`   ❌ ${endpoint.name} unexpected response`);
      }
    } catch (error) {
      console.log(`   ❌ ${endpoint.name} failed: ${(error as Error).message}`);
  ...
  // Additional check: try to ping the database hostname directly
  ...
  try {
    ...
    console.log(`Database hostname test failed: ${(error as Error).message}`);
  }
  ...
  if (projectExists) {
    console.log(
      "✅ Supabase project appears to exist and is accessible via API"
    );
    ...
    console.log(
      "   3. Network routing problem between your location and database"
    );
    ...
    console.log(
      "❌ Supabase project does not appear to exist or is inaccessible"
    );
