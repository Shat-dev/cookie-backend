...
/**
 * Test Suite for automatedLottery Service
 *
 * Tests the following functionality:
 * 1. Service lifecycle - Start/stop operations
 * 2. Round state tracking - Active, completed, freeze states
 * 3. Freeze window timing - 3-minute freeze before round end
 * 4. Snapshot creation - Deterministic entry ordering
 * 5. Recovery mechanism - Handles server downtime during freeze
 * 6. Auto-round creation - Creates next round if pool has entries
 * 7. VRF integration - Waits for Chainlink automation
 * 8. Winner detection - Extracts winner from completed rounds
 */
...
async function runTests() {
  ...
  try {
    ...
    automatedLotteryService.start(); // Should log "already running"
    ...
    console.log(
      `  Round end: ${new Date(mockRound.end * 1000).toLocaleTimeString()}`
    );
    console.log(
      `  Freeze start: ${new Date(
        freezeStart * 1000
      ).toLocaleTimeString()} (${FREEZE_SEC}s before end)`
    );
    console.log(
      `  Freeze end: ${new Date(
        freezeEnd * 1000
      ).toLocaleTimeString()} (${SAFETY_SEC}s before end)`
    );
    ...
    const entries = [
      ...
      { wallet_address: "0xAAA", token_id: "10" }, // Numeric comparison
    ];
    ...
    console.log(
      "  Before sort:",
      entries.map((e) => `${e.wallet_address}:${e.token_id}`).join(", ")
    );
    console.log(
      "  After sort:",
    ...
    console.log(
      `  Round ${roundNumber} snapshot: ${existingSnapshot || "Not found"}`
    );
    ...
    // Test 6: Auto-round creation logic
    ...
    // Test 7: Winner extraction
    console.log("Test 7: Winner extraction");
    const mockWinner = {
      address: "0x1234567890123456789012345678901234567890",
      tokenId: 123n,
      displayId: 123n, // Already human-readable
    };
    ...
    // Test 8: Tick interval
    console.log("Test 8: Tick interval");
    const checkInterval = Number(process.env.AUTOMATION_CHECK_MS) || 10_000;

    console.log(
      `  Check interval: ${checkInterval}ms (${checkInterval / 1000}s)`
    );
